...
\begin{itemize}
    \item \textbf{RMSE (Root Mean Square Error)}: Mide la magnitud promedio del error en las predicciones
    \item \textbf{MAPE (Mean Absolute Percentage Error)}: Expresa el error como porcentaje del valor real
    \item \textbf{MAE (Mean Absolute Error)}: Calcula el promedio de los errores absolutos
\end{itemize}
...
El sistema implementa un enfoque de \textbf{ensemble learning} que combina cuatro algoritmos complementarios:

\begin{enumerate}
    \item \textbf{Random Forest Regressor}: 100 árboles, profundidad máxima 10, características: precio, volumen, indicadores técnicos (RSI, MACD, Bollinger Bands)

    \item \textbf{XGBoost}: Learning rate 0.1, 100 estimadores, max depth 5, características: precio OHLC, volumen, medias móviles (7, 14, 30 días)

    \item \textbf{LSTM (Long Short-Term Memory)}: 2 capas LSTM (50 unidades cada una), dropout 0.2, ventana de secuencia 60 días

    \item \textbf{Prophet (Facebook)}: Componentes de tendencia y estacionalidad, series temporales de precios de cierre, horizonte de 5 días
\end{enumerate}

\subsection{Resultados de Evaluación Individual}

La Tabla~\ref{tab:modelos_ml} presenta las métricas obtenidas para cada modelo en el conjunto de validación. El modelo ensemble supera a todos los modelos individuales, logrando un RMSE de \$2.64 y un MAPE de 1.71\%.

\begin{table}[htbp]
\centering
\caption{Métricas de Evaluación de Modelos ML}
\label{tab:modelos_ml}
\begin{tabular}{lcccc}
\hline
\textbf{Modelo} & \textbf{RMSE (\$)} & \textbf{MAPE (\%)} & \textbf{MAE (\$)} & \textbf{Tiempo (s)} \\
\hline
Random Forest & 3.42 & 2.18 & 2.87 & 0.12 \\
XGBoost & 2.89 & 1.93 & 2.31 & 0.08 \\
LSTM & 4.15 & 2.67 & 3.52 & 1.85 \\
Prophet & 3.78 & 2.34 & 3.12 & 0.45 \\
\textbf{Ensemble} & \textbf{2.64} & \textbf{1.71} & \textbf{2.08} & \textbf{2.50} \\
\hline
\end{tabular}
\end{table}

La Figura~\ref{fig:rmse_mape} muestra la comparación visual de los errores RMSE y MAPE entre los diferentes modelos.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{graficos/grafico_rmse_mape.pdf}
\caption{Comparación de errores RMSE y MAPE por modelo}
\label{fig:rmse_mape}
\end{figure}
...
\begin{itemize}
    \item XGBoost demostró ser el modelo individual más preciso, con el menor RMSE (2.89) y MAPE (1.93\%)
    \item LSTM presenta mayor error pero captura mejor patrones temporales complejos
    \item Prophet es efectivo para capturar tendencias y estacionalidad
    \item El enfoque ensemble supera a todos los modelos individuales, reduciendo el RMSE en 8.7\% respecto a XGBoost
    \item El tiempo de inferencia del ensemble (2.5s) es aceptable para aplicaciones en tiempo real
\end{itemize}

\subsection{Análisis por Ticker}

Se evaluó el rendimiento del ensemble en diferentes tipos de acciones. La Tabla~\ref{tab:resultados_ticker} muestra que las acciones del sector financiero presentan los menores errores de predicción, mientras que las acciones de alta volatilidad (tecnológicas) tienen errores significativamente mayores.
...
\hline
\textbf{Ticker} & \textbf{Sector} & \textbf{Volatilidad} & \textbf{RMSE} & \textbf{MAPE (\%)} & \textbf{Observaciones} \\
\hline
...
Se realizó validación cruzada temporal (5 folds) para evaluar la robustez de los modelos. Los resultados se presentan en la Tabla~\ref{tab:validacion_cruzada}.
...
\begin{tabular}{lccc}
\hline
\textbf{Modelo} & \textbf{MAPE Promedio} & \textbf{Desv. Estándar} & \textbf{Coef. Variación} \\
\hline
Random Forest & 2.21\% & 0.34\% & 15.4\% \\
XGBoost & 1.96\% & 0.28\% & 14.3\% \\
LSTM & 2.71\% & 0.52\% & 19.2\% \\
Prophet & 2.38\% & 0.41\% & 17.2\% \\
\textbf{Ensemble} & \textbf{1.73\%} & \textbf{0.23\%} & \textbf{13.3\%} \\
\hline
\end{tabular}
\end{table}
...
Los cuellos de botella identificados son: ModelAgent (49.4\%) debido a LSTM y Prophet, y MarketAgent (38.9\%) por la dependencia de API externa (yfinance).
...
\begin{itemize}
    ...
    \item Predicción 5 días: \$189.80 (+1.25\%)
...
\textbf{Resultado real (5 días después):}
\begin{itemize}
    \item Precio final: \$191.20
    \item Ganancia: +2.0\% (\$40)
    \item Sistema predijo: +1.25\% (error: 0.75 puntos porcentuales)
    \item El sistema acertó la dirección (alcista) ✓
\end{itemize}
...
\begin{itemize}
    ...
    \item Sistema predijo: +2.8\% (error: 0.6 puntos porcentuales)
...
\hline
Principiante & Ganancia & +2.0\% real & Dirección correcta & Recomendación clara \\
Trader & Timing & +3.4\% & Sentimiento en tiempo real & Alert de cambio rápido \\
Gestor & Diversificación & +0.55\% alpha & Análisis comparativo & Priorización inteligente \\
Alertas & Protección & Evitó pérdida & Detección temprana & Notificación automática \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Modelos de Machine Learning Efectivos}: Ensemble learning con MAPE de 1.71\%, superando modelos individuales. Mejor desempeño en acciones de baja volatilidad (MAPE < 1.5\%). Validación cruzada demuestra alta consistencia (coeficiente de variación: 13.3\%).
...
Predicciones < 5\% error (MAPE) & < 5\% & 1.71\% ✓ \\
...
\begin{itemize}
    ...
    \item \textbf{Precisión Variable Según Volatilidad}: Acciones de alta volatilidad (TSLA, NVDA) presentan MAPE > 4\%, eventos imprevistos (cisnes negros) no pueden ser predichos.
...
\begin{enumerate}
    ...
    \item Ensemble learning temporal con integración de modelos diversos (RF, XGBoost, LSTM, Prophet)
...
\begin{enumerate}
    ...
    \item \textbf{Enriquecimiento de Datos}: Agregar indicadores técnicos adicionales (ADX, Ichimoku), integrar datos de volumen de opciones, incluir datos fundamentales (P/E ratio, EPS). \textit{Impacto esperado}: Mejora de MAPE de 1.71\% a < 1.5\%.
